                ...
                <section class="log__inner">
                        ...
                        try {
                            ...
                            if ($user['user_ver_key'] == $verification_key) {
                                //verify user and delete verification key
                                $verify_user = $db->prepare("UPDATE users SET user_is_verified=1, user_ver_key='' WHERE user_id=:user_id");
                                $verify_user->bindValue(":user_id", $user_id);
                                $verify_user->execute();

                                //update also in session that user is verified
